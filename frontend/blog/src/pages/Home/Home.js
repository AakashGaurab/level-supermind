...
function Home() {
  ...
  useEffect(()=>{
    ...
    async function get_data(){
      try {
        let response = await fetch("http://level-supermind-431007708.ap-south-1.elb.amazonaws.com:3500/blogs");
        let final_response = await response.json()
        console.log(final_response);
...
/* *****************************************fetching comments************************************ */
  ...
  let get_comment=async(id)=>{                         //getting all comments related to particular blog
    setBlogid(id); 
    try {
      let response = await fetch(`http://level-supermind-431007708.ap-south-1.elb.amazonaws.com:3500/comment/${id}`);
      let final_response = await response.json();
  ...
  async function post_comment(data){                         //posting comment to backend
    try {
      let response = await fetch(`http://level-supermind-431007708.ap-south-1.elb.amazonaws.com:3500/comment/${id}/${blogid}`,{
        method:"POST",
        headers:{
          "Content-type":"application/json"
        },
